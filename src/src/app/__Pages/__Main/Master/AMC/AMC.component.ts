...
export class AMCComponent implements OnInit {
    ...
    switch (__items.flag) {
      ...
      case 'U':
        ...
        this.__utility.navigatewithqueryparams(__items.url, {
          queryParams: {
            product_id: this.route.snapshot.queryParamMap.get('product_id'),
          },
        });
  ...
  openDialog(__amc: amc | null = null, __amcId: number) {
    ...
    dialogConfig.data = {
      ...
      product_id: this.route.snapshot.queryParamMap.get('product_id')
        ? atob(this.route.snapshot.queryParamMap.get('product_id'))
        : '',
  ...
  openDialogForReports(
    ...
    try {
      const dialogref = this.__dialog.open(AmcrptComponent, dialogConfig);
    } catch (ex) {
      const dialogRef = this.__dialog.getDialogById(dialogConfig.id);
      dialogRef.addPanelClass('mat_dialog');
      this.__utility.getmenuIconVisible({
