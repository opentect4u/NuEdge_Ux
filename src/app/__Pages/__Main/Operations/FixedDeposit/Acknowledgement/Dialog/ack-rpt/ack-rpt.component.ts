...
export class AckRPTComponent implements OnInit {
  ...
  setColumns(options) {
    ...
    this.__columns = fdTraxClm.Columns.filter(item => item.isVisible.includes(Number(options))).filter(item => !['edit'].includes(item.field))
  ...
  ngAfterViewInit() {
    ...
    this.__insTraxForm.controls['comp_type_id'].valueChanges.subscribe(
      (res) => {
        this.getSchemeMst(this.__insTraxForm.value.company_id, res);
    ...
    this.__insTraxForm.controls['company_id'].valueChanges.subscribe((res) => {
      this.getSchemeMst(res,this.__insTraxForm.value.comp_type_id);
    });
    // /*** END */
  }
  searchInsurance() {
