...
export class AckSearchRPTComponent implements OnInit {
  ...
  setColumns(){
       this.__columns = fdTraxClm.COLUMNFORDETAILS.filter((x: any) => x!= 'delete');
  }
  ...
  getFDMstRPT(column_name: string | null | undefined = '',sort_by: string | null | undefined = 'asc'){
    ...
    __fd.append('investor_name',global.getActualVal(this.__insTraxForm.value.investor_code));
    ...
    this.__dbIntr.api_call(1,'/fd/ackDetailSearch',__fd).pipe(pluck("data")).subscribe((res: any) =>{
  ...
  fullScreen(){
        ...
        this.__dbIntr
          .getpaginationData(
            __paginate.url +
              ...
              +('&investor_name=' + this.__insTraxForm.value.options == '3' ? '' : global.getActualVal(this.__insTraxForm.value.investor_code))
    ...
    updateRow(row_obj){
     ...
     finalSubmitAck(){
      ...
      this.__dbIntr.api_call(1,'/fd/ackFinalSubmit',__finalSubmit).subscribe((res: any) => {
