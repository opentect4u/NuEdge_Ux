...
@Component({
  selector: 'app-nfo-manual-update',
  templateUrl: './nfo-manual-update.component.html',
  styleUrls: ['./nfo-manual-update.component.css']
})
export class NfoManualUpdateComponent implements OnInit {
  __transType: any = [];
...
__euinMst: any = [];
  ...
  private getAMCwiseScheme(amc_ids){
    ...
    customSort(ev){
      ...
      this.clmList = clmn
      if(option == 2){
           this.__columns = (trans_id == 5) ? global.getColumnsAfterMerge(MfackClmns.Summary_common ,MfackClmns.Summary_Sip)
           : global.getColumnsAfterMerge(MfackClmns.Summary_common ,MfackClmns.Summary_Pip_Switch)
      }
      else{
    ...
    onselectItem(__itemsPerPage) {
      ...
      populateDT(__items) {
          ...
          dialogConfig.data = {
            ...
            data: __items,
            id :'MUNFO_' + (__items.tin_no ? __items.tin_no.toString() : '0')
          };
          dialogConfig.id =
            'MUNFO_' + (__items.tin_no ? __items.tin_no.toString() : '0');
          try {
