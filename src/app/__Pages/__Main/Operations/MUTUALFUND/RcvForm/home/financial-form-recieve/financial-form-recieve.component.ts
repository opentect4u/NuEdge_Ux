...
@Component({
  selector: 'financial-form-recieve',
  templateUrl: './financial-form-recieve.component.html',
  styleUrls: ['./financial-form-recieve.component.css']
})
export class FinancialFormRecieveComponent implements OnInit {

  @Input() trans_type_id:string;
  product_id:string = '1'
  ...
  recieveForm() {
    ...
    __rcvForm.append('product_id', this.product_id);
    ...
    this.__dbIntr
      ...
      .subscribe((res: any) => {
        if(res?.suc == 1){
          ...
          this.__utility.showSnackbar(
            res.suc == 1
              ? 'Form with temporary TIN ' + res.data.temp_tin_no + ' has been received successfully': 'Something went wrong! Plase try again later ',
             res.suc
          );
      });
  }
  ...
  resetForm = () =>{
    ...
    this.__dialogDtForScheme = null;
    this.__dialogDtForSchemeTo = null;
    this.__rcvForm.get('sub_arn_no').setValue('',{emitEvent:false});
    this.__rcvForm.get('euin_no').setValue('',{emitEvent:false});
    this.__rcvForm.get('client_code').setValue('',{emitEvent:false});
    this.__rcvForm.get('scheme_name').setValue('',{emitEvent:false});
    this.__rcvForm.get('switch_scheme_to').setValue('',{emitEvent:false});
    this.__rcvForm.patchValue({
      sub_brk_cd:'',
      client_name:'',
      client_id:'',
