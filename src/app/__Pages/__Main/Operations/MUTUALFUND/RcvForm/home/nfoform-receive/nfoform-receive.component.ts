...
@Component({
  selector: 'nfoform-receive',
  templateUrl: './nfoform-receive.component.html',
  styleUrls: ['./nfoform-receive.component.css']
})
export class NFOFormReceiveComponent implements OnInit {
...
getTransactionType(){
  this.__dbIntr.api_call(0,'/showTrans','trans_type_id='+this.trans_type_id).pipe(pluck("data")).subscribe((res:any) => {
    console.log(res);
  ...
  pipe(
    ...
    error: (err) => {this.__isClientPending = false;}
      ...
      pipe(
        ...
        distinctUntilChanged(),
        switchMap(dt => dt?.length > 1 ?
          this.__dbIntr.searchItems('/scheme', (dt + '&scheme_type='+ (this.trans_type_id == '4' ? 'N' : 'O')))
          : []),
        map((x: any) => x.data)
      ).subscribe({
        next: (value) => {
