...
export class EntryComponent implements OnInit {
  ...
  renewal_bu_oportunity = new FormGroup({
    temp_tin_no: new FormControl(
      this.data.data ? this.data.data.temp_tin_no : ''
    ),
    ...
    product_id: new FormControl(
      this.data.data ? this.data.data.product_id : '',
      [Validators.required]
    ),
    sum_insured: new FormControl(
      this.data.data ? this.data.data.sum_insured : '',
      [Validators.required]
    ),
    renewal_dt: new FormControl(
      this.data.data ? this.data.data.renewal_dt : '',
      [Validators.required]
    ),
    upload_scan: new FormControl(''),
    preview_file: new FormControl(
      this.data.data
        ? `${environment.renwal_bu_frm_url + this.data.data.upload_file}`
        : ''
    ),
    upload_file: new FormControl(''),
    remarks: new FormControl(this.data.data ? this.data.data.remarks : ''),
  });
  ...
  ngOnInit(): void {
    ...
    this.populateRenewal(this.data.data);
  }
  populateRenewal(buOpportunity) {
    if (this.data.data) {
      setTimeout(() => {
        this.renewal_bu_oportunity.patchValue({
          bu_type: buOpportunity.bu_type,
          ins_type_id: buOpportunity.ins_type_id,
          comp_id: buOpportunity.comp_id,
          product_type_id: buOpportunity.product_type_id,
        });
        this.__clientMst.push({
          id: buOpportunity.proposer_id,
          client_name: buOpportunity.proposer_name,
          client_code: buOpportunity.proposer_code,
          dob: buOpportunity.proposer_dob,
        });
        this.__empMst.push({
          euin_no: buOpportunity.euin_no,
          emp_name: buOpportunity.emp_name,
        });
        this.getItems(this.__empMst[0], 'E');
        this.getItems(this.__clientMst[0], 'C');
        if (buOpportunity.same_as_above == 'true') {
          this.renewal_bu_oportunity.controls['same_as_above'].reset(
            buOpportunity.same_as_above == 'true' ? true : false,
            { emitEvent: true }
          );
        } else {
          this.getItems(
            {
              id: buOpportunity.insured_person_id,
              client_name: buOpportunity.insured_person_name,
              client_code: buOpportunity.insured_person_code,
              pan: buOpportunity.insured_person_pan,
              dob: buOpportunity.insured_person_dob,
            },
            'I'
          );
        }
        if (buOpportunity.bu_type == 'B') {
          this.__subArnMst.push({ arn_no: buOpportunity.arn_no });
          this.getItems(this.__subArnMst[0], 'S');
        }
      }, 200);
    }
  }
  ngAfterViewInit() {
    this.renewal_bu_oportunity.controls['bu_type'].valueChanges.subscribe(
      (res) => {
        this.renewal_bu_oportunity.controls['sub_arn_no'].setValidators(
          res == 'B' ? [Validators.required] : null
        );
        this.renewal_bu_oportunity.controls['sub_arn_no'].setAsyncValidators(
          res == 'B' ? [this.SubBrokerValidators()] : null
        );
        this.renewal_bu_oportunity.controls[
          'sub_arn_no'
        ].updateValueAndValidity({ emitEvent: false });
      }
    );

    this.renewal_bu_oportunity.controls['same_as_above'].valueChanges.subscribe(
      (res) => {
        this.getItems(res ? this.singleProposerDtls : '', 'I');
      }
    );

    this.renewal_bu_oportunity.controls['euin_no'].valueChanges
      .pipe(
    ...
    this.renewal_bu_oportunity.controls['proposer_code'].valueChanges
      ...
      .subscribe({
        next: (value) => {
          ...
          this.renewal_bu_oportunity.controls['proposer_name'].reset('', {
            emitEvent: false,
          });
          this.renewal_bu_oportunity.controls['proposer_id'].reset('', {
            emitEvent: false,
          });
    ...
    this.renewal_bu_oportunity.controls['ins_person_code'].valueChanges
      .pipe(
        tap(() => (this.__isProposerPending = true)),
        debounceTime(200),
        distinctUntilChanged(),
      ...
      .subscribe({
        next: (value) => {
          ...
          this.renewal_bu_oportunity.controls['ins_person_name'].reset('', {
            emitEvent: false,
          });
          this.renewal_bu_oportunity.controls['insured_person_id'].reset('', {
            emitEvent: false,
          });
          this.renewal_bu_oportunity.controls['ins_person_pan'].reset('', {
            emitEvent: false,
          });
          this.renewal_bu_oportunity.controls['ins_person_dob'].reset('', {
            emitEvent: false,
          });
    ...
    this.renewal_bu_oportunity.controls['ins_type_id'].valueChanges.subscribe(
      (res) => {
        this.getCompany(res);
      }
    );
    this.renewal_bu_oportunity.controls['comp_id'].valueChanges.subscribe(
      (res) => {
  ...
  getItems(item, flag) {
    switch (flag) {
      case 'E':
        this.renewal_bu_oportunity.controls['euin_no'].reset(
          item ? item.emp_name + '-' + item.euin_no : '',
          { emitEvent: false }
        );
        this.searchResultVisibilityForEmployee('none');
        break;
      case 'S':
        this.renewal_bu_oportunity.controls['sub_arn_no'].reset(
          item ? item.arn_no : '',
          { emitEvent: false }
        );
      ...
      case 'C':
        ...
        this.renewal_bu_oportunity.controls['proposer_code'].reset(
        ...
        break;
      case 'I':
        this.renewal_bu_oportunity.controls['ins_person_code'].reset(
          item ? item.client_code : '',
          { emitEvent: false }
        );
        this.renewal_bu_oportunity.controls['ins_person_name'].reset(
  ...
  getCompany(__res) {
    if (__res) {
      ...
      this.renewal_bu_oportunity.controls['comp_id'].setValue('', {
        emitEvent: true,
      });
  ...
  getProductType(__res) {
    if (__res) {
      ...
      this.renewal_bu_oportunity.controls['product_type_id'].setValue('', {
        emitEvent: true,
      });
  ...
  getProductName(__prd_type_id) {
    if (__prd_type_id) {
      this.__dbIntr
        .api_call(0, '/ins/product', +'&product_type_id=' + __prd_type_id)
        .pipe(pluck('data'))
        .subscribe((res: insProduct[]) => {
          this.__prdMst = res;
        });
    } else {
      this.renewal_bu_oportunity.controls['product_id'].setValue('', {
        emitEvent: true,
      });
      this.__prdMst = [];
    }
  ...
  getFile(ev) {
    ...
    if (
      this.renewal_bu_oportunity.get('upload_scan').status == 'VALID' &&
      ev.files.length > 0
    ) {
  ...
  submitRenewal() {
    ...
    res['euin_no'] = global.containsSpecialChars(
      this.renewal_bu_oportunity.value.euin_no
    )
      ? this.renewal_bu_oportunity.value.euin_no.split('-')[1]
      : this.renewal_bu_oportunity.value.euin_no;
    this.__dbIntr
      .api_call(1, '/ins/businessOpportunityAddEdit', this.convertFormData(res))
      .subscribe((res: any) => {
        if (res.suc == 1) {
          this.dialogRef.close({ data: res.data,temp_tin_no:this.data.data ? this.data.data.temp_tin_no : ''});
        }
  ...
  existingClient() {
    this.getItems('', 'I');
    this.renewal_bu_oportunity.controls['same_as_above'].setValue(false, {
      emitEvent: false,
    });
  ...
  openDialog(__type) {
    ...
    dialogConfig.data = {
      flag: __type,
      title: __type == 'C' ? this.singleProposerDtls.client_name : '',
      dt: __type == 'C' ? this.singleProposerDtls : '',
    };
    try {
  ...
  openPDF() {
    window.open(this.renewal_bu_oportunity.value.preview_file, '_blank');
  }
}
