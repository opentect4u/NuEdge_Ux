...
export class AckEntryComponent implements OnInit {
  ...
  __ackUpload = new FormGroup({
    login_cutt_off: new FormControl(this.data.data.comp_login_cutt_off ? this.data.data.comp_login_cutt_off : ''),
    comp_login_dt: new FormControl(this.data.data.comp_login_dt ? this.data.data.comp_login_dt.split(' ')[0] : '',[Validators.required]),
    ack_file: new FormControl('',[Validators.required,fileValidators.fileExtensionValidator(this.allowedExtensions)]),
    comp_login_time: new FormControl(this.data.data.comp_login_dt ? this.data.data.comp_login_dt.split(' ')[1] : '',[Validators.required]),
    remarks: new FormControl(this.data.data.ack_remarks ? this.data.data.ack_remarks : ''),
    file: new FormControl(this.data.data.ack_copy_scan ? `${environment.ack_formUrl_for_ins + this.data.data.ack_copy_scan}` : '')
  })
  constructor(
  ...
  getcurrenctDatetime(){
    ...
    UploadAcknowledgement(){
      ...
      __ackUpload.append('comp_login_dt',this.__ackUpload.value.comp_login_dt);
      __ackUpload.append('ack_copy_scan',this.__ackUpload.value.file);
      __ackUpload.append('comp_login_time',this.__ackUpload.value.comp_login_time);
      __ackUpload.append('ack_remarks',this.__ackUpload.value.remarks);
      this.__dbIntr.api_call(1,'/ins/ackUpload',__ackUpload).subscribe((res: any) =>{
