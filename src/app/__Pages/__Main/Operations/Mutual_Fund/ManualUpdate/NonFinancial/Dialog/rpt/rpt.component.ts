...
export class RPTComponent implements OnInit {
  ...
  populateDT(__items) {
    ...
    dialogConfig.id = 'FDMUNOFIN_' + (__items.tin_no ? __items.tin_no.toString() : '0');
    try {
  ...
  getNONFINRPT(column_name: string | null = '',sort_by: string | null | '' ='asc'){
    const __mfTrax = new FormData();
  ...
  submit() {
     this.getNONFINRPT(this.__sortAscOrDsc.active,this.__sortAscOrDsc.direction);
  }
  exportPdf() {
    if(this.__rcvForms.get('options').value == '3'){
