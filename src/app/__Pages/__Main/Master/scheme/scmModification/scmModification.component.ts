...
export class ScmModificationComponent implements OnInit {
  ...
  __scmForm = new FormGroup({
      ...
      step_up_min_per: new FormControl(this.data.id > 0 ? this.data.items.step_up_min_per : (global.getActualVal(this.__getPrevScmDT) ? this.__getPrevScmDT.step_up_min_per : '')),
      step_up_min_amt: new FormControl(this.data.id > 0 ? this.data.items.step_up_min_amt : (global.getActualVal(this.__getPrevScmDT) ? this.__getPrevScmDT.step_up_min_amt : '')),
    ...
    nfo_entry_date: new FormControl(this.data.id > 0 ? global.getActualVal(this.data.items.nfo_entry_date) : (global.getActualVal(this.__getPrevScmDT) ? this.__getPrevScmDT.nfo_entry_date : ''),this.data.scheme_type == 'N' ? [Validators.required] : []),
    ...
    ava_special_sip: new FormControl(
      ...
      ava_special_swp: new FormControl(
        this.data.id > 0 ?
        (this.data.items.ava_special_swp == "true" ? true : false)
        : (
          (global.getActualVal(this.__getPrevScmDT) ?
        ...
        ava_special_stp: new FormControl(
          this.data.id > 0 ?
          (this.data.items.ava_special_stp == "true" ? true : false)
          : (
            (global.getActualVal(this.__getPrevScmDT) ?
    ...
    special_swp_name: new FormControl(this.data.id > 0 ? this.data.items.special_swp_name : (global.getActualVal(this.__getPrevScmDT) ? this.__getPrevScmDT.special_swp_name : '')),
    special_stp_name: new FormControl(this.data.id > 0 ? this.data.items.special_stp_name : (global.getActualVal(this.__getPrevScmDT) ? this.__getPrevScmDT.special_stp_name : '')),

    nfo_one_pager_upload: new FormControl('',this.data.scheme_type == 'N' ? [fileValidators.fileExtensionValidator(this.allowedExtensions)] : [] ),
    nfo_one_pager_file: new FormControl(''),
    nfo_one_pager_filePreview: new FormControl(''),

    nfo_kim_key_memorandom: new FormControl('',this.data.scheme_type == 'N' ? [fileValidators.fileExtensionValidator(this.allowedExtensions)] : [] ),
    nfo_kim_key_file: new FormControl(''),
    nfo_kim_key_filePreview: new FormControl(''),

    nfo_ppt: new FormControl('',this.data.scheme_type == 'N' ? [fileValidators.fileExtensionValidator(this.allowedExtensions)] : [] ),
    nfo_ppt_file: new FormControl(''),
    nfo_ppt_filePreview: new FormControl(''),

    common_application_form: new FormControl('',this.data.scheme_type == 'N' ? [fileValidators.fileExtensionValidator(this.allowedExtensions)] : [] ),
    common_application_file: new FormControl(''),
    common_application_filePreview: new FormControl(''),

    sip_reg_form: new FormControl('',this.data.scheme_type == 'N' ? [fileValidators.fileExtensionValidator(this.allowedExtensions)] : [] ),
    sip_reg_file: new FormControl(''),
    sip_reg_filePreview: new FormControl(''),


    swp_reg_form: new FormControl('',this.data.scheme_type == 'N' ? [fileValidators.fileExtensionValidator(this.allowedExtensions)] : [] ),
    swp_reg_file: new FormControl(''),
    swp_reg_filePreview: new FormControl(''),

    stp_reg_form: new FormControl('',this.data.scheme_type == 'N' ? [fileValidators.fileExtensionValidator(this.allowedExtensions)] : [] ),
  ...
  ngOnInit() {
    this.__scmForm.controls['amc_id'].updateValueAndValidity({emitEvent:true});
  ...
  setOthers(id, name, doc) {
    return new FormGroup({
      ...
      doc_name: new FormControl('', [fileValidators.fileExtensionValidator(this.allowedExtensions)]),
      file_preview: new FormControl(doc!= null ? `${environment.clientdocUrl}` + doc : ''),
      form_upload: new FormControl(doc!= null ? `${environment.clientdocUrl}` + doc : '')
    });
  }
   ...
   if (this.__scmForm.get('is_selectall_for_swp').value !== allSelected) {
     this.__scmForm.get('is_selectall_for_swp').patchValue(allSelected, { emitEvent: false });
      ...
      this.__scmForm.controls['ava_special_swp'].valueChanges.subscribe(res =>{
        this.__scmForm.controls['special_swp_name'].setValidators(res ? [Validators.required] : null);
        this.__scmForm.controls['special_swp_name'].updateValueAndValidity({emitEvent: false});
        if(!res){
     ...
     this.__scmForm.controls['ava_special_stp'].valueChanges.subscribe(res =>{
      this.__scmForm.controls['special_stp_name'].setValidators(res ? [Validators.required] : null);
      this.__scmForm.controls['special_stp_name'].updateValueAndValidity({emitEvent: false});
      if(!res){
     ...
     //Same As SIP DATES
     this.__scmForm.controls['same_as_sip_dates'].valueChanges.subscribe(res =>{
          this.__scmForm.controls['sip_date'].setValue(res ? (this.__scmDtls.length > 0 ? JSON.parse(this.__scmDtls[0].sip_date) : []) : [])
     })

     //Same As STP DATES
     this.__scmForm.controls['same_as_stp_dates'].valueChanges.subscribe(res =>{
      this.__scmForm.controls['stp_date'].setValue(res ? (this.__scmDtls.length > 0 ? JSON.parse(this.__scmDtls[0].stp_date) : []) : [])

     })

      //Same As SWP DATES
      this.__scmForm.controls['same_as_swp_dates'].valueChanges.subscribe(res =>{
        this.__scmForm.controls['swp_date'].setValue(res ? (this.__scmDtls.length > 0 ? JSON.parse(this.__scmDtls[0].swp_date) : []) : [])
  ...
  submit(){
    ...
    if(this.data.scheme_type == 'N'){
      __scm.append("nfo_ppt",  global.getActualVal(this.__scmForm.value.nfo_ppt_file));
      __scm.append("nfo_kim", global.getActualVal(this.__scmForm.value.nfo_kim_key_file));
      __scm.append("nfo_one_pager",  global.getActualVal(this.__scmForm.value.nfo_one_pager_file));
      __scm.append("nfo_common_app",  global.getActualVal(this.__scmForm.value.common_application_file));
      __scm.append("sip_registration",global.getActualVal(this.__scmForm.value.sip_reg_file));
      __scm.append("swp_registration",global.getActualVal(this.__scmForm.value.swp_reg_file));
      __scm.append("stp_registration",global.getActualVal(this.__scmForm.value.stp_reg_file));


      for (let i = 0; i < this.__scmForm.value.others.length; i++) {
        if (typeof (this.__scmForm.value.others[i].file) != 'string') {
          __scm.append("form_upload[]", global.getActualVal(this.__scmForm.value.others[i].form_upload));
          __scm.append("row_id[]", global.getActualVal(this.__scmForm.value.others[i].id));
          __scm.append("form_name[]", global.getActualVal(this.__scmForm.value.others[i].form_name));
        }
      }
    }
    ...
    __scm.append('step_up_min_amt',global.getActualVal(this.__scmForm.value.step_up_min_amt));
    __scm.append('step_up_min_per',global.getActualVal(this.__scmForm.value.step_up_min_per));
    ...
    __scm.append("special_swp_name", this.__scmForm.value.ava_special_swp ? this.__scmForm.value.special_swp_name : '');

    __scm.append("ava_special_stp", this.__scmForm.value.ava_special_stp);
    __scm.append("special_stp_name", this.__scmForm.value.ava_special_stp ? this.__scmForm.value.special_stp_name : '');
  ...
  getFile(__ev,mode){
    switch(mode){
      case 'O':
        this.__scmForm.controls['nfo_one_pager_upload'].setValidators([fileValidators.fileSizeValidatorforEntry(__ev.files), fileValidators.fileExtensionValidator(this.allowedExtensions)])
        this.__scmForm.controls['nfo_one_pager_upload'].updateValueAndValidity();
        if (this.__scmForm.controls['nfo_one_pager_upload'].status == 'VALID' && __ev.files.length > 0) {
          this.__scmForm.controls['nfo_one_pager_file'].setValue(__ev.files[0]);
          this.__scmForm.controls['nfo_one_pager_filePreview'].patchValue(this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL( __ev.files[0])))
        }
        else {
        ...
        break;
      case 'K':
        this.__scmForm.controls['nfo_kim_key_memorandom'].setValidators([fileValidators.fileSizeValidatorforEntry(__ev.files), fileValidators.fileExtensionValidator(this.allowedExtensions)])
        this.__scmForm.controls['nfo_kim_key_memorandom'].updateValueAndValidity();
        if (this.__scmForm.controls['nfo_kim_key_memorandom'].status == 'VALID' && __ev.files.length > 0) {
          this.__scmForm.controls['nfo_kim_key_file'].setValue(__ev.files[0]);
          this.__scmForm.controls['nfo_kim_key_filePreview'].patchValue(this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL( __ev.files[0])))
        }
        else {
      ...
      break;
      case 'P':
        this.__scmForm.controls['nfo_ppt'].setValidators([fileValidators.fileSizeValidatorforEntry(__ev.files), fileValidators.fileExtensionValidator(this.allowedExtensions)])
        this.__scmForm.controls['nfo_ppt'].updateValueAndValidity();
        if (this.__scmForm.controls['nfo_ppt'].status == 'VALID' && __ev.files.length > 0) {
          this.__scmForm.controls['nfo_ppt_file'].setValue(__ev.files[0]);
          this.__scmForm.controls['nfo_ppt_filePreview'].patchValue(this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL( __ev.files[0])))
        }
        else {
      ...
      break;
      case 'C':
        this.__scmForm.controls['common_application_form'].setValidators([fileValidators.fileSizeValidatorforEntry(__ev.files), fileValidators.fileExtensionValidator(this.allowedExtensions)])
        this.__scmForm.controls['common_application_form'].updateValueAndValidity();
        if (this.__scmForm.controls['common_application_form'].status == 'VALID' && __ev.files.length > 0) {
          this.__scmForm.controls['common_application_file'].setValue(__ev.files[0]);
          this.__scmForm.controls['common_application_filePreview'].patchValue(this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL( __ev.files[0])))
        }
        else {
      ...
      break;
      case 'I':
        this.__scmForm.controls['sip_reg_form'].setValidators([fileValidators.fileSizeValidatorforEntry(__ev.files), fileValidators.fileExtensionValidator(this.allowedExtensions)])
        this.__scmForm.controls['sip_reg_form'].updateValueAndValidity();
        if (this.__scmForm.controls['sip_reg_form'].status == 'VALID' && __ev.files.length > 0) {
          this.__scmForm.controls['sip_reg_file'].setValue(__ev.files[0]);
          this.__scmForm.controls['sip_reg_filePreview'].patchValue(this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL( __ev.files[0])))
        }
        else {
      ...
      break;
      case 'W':
        this.__scmForm.controls['swp_reg_form'].setValidators([fileValidators.fileSizeValidatorforEntry(__ev.files), fileValidators.fileExtensionValidator(this.allowedExtensions)])
        this.__scmForm.controls['swp_reg_form'].updateValueAndValidity();
        if (this.__scmForm.controls['swp_reg_form'].status == 'VALID' && __ev.files.length > 0) {
          this.__scmForm.controls['swp_reg_file'].setValue(__ev.files[0]);
          this.__scmForm.controls['swp_reg_filePreview'].patchValue(this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL( __ev.files[0])))
        }
        else {
      ...
      break;
      case 'T':
        this.__scmForm.controls['stp_reg_form'].setValidators([fileValidators.fileSizeValidatorforEntry(__ev.files), fileValidators.fileExtensionValidator(this.allowedExtensions)])
        this.__scmForm.controls['stp_reg_form'].updateValueAndValidity();
        if (this.__scmForm.controls['stp_reg_form'].status == 'VALID' && __ev.files.length > 0) {
          this.__scmForm.controls['stp_reg_file'].setValue(__ev.files[0]);
          this.__scmForm.controls['stp_reg_filePreview'].patchValue(this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL( __ev.files[0])))
        }
        else {
  ...
  getOtherFiles(__ev, index, __type_id) {
    this.others.controls[index].get('doc_name').setValidators([fileValidators.fileSizeValidatorforEntry(__ev.target.files), fileValidators.fileExtensionValidator(this.allowedExtensions)])
    this.others.controls[index].get('doc_name').updateValueAndValidity();
    if (this.others.controls[index].get('doc_name').status == 'VALID') {
      this.others.controls[index].get('file_preview')?.patchValue(this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL( __ev.target.files[0])));
      this.others.controls[index].get('form_upload')?.patchValue(__ev.target.files[0]);
    }
    else {
