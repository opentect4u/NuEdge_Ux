...
export type totaltrxnSummaryFooterTotal = {
  ...
  @Component({
    selector: 'app-rejct-transaction',
    templateUrl: './rejct-transaction.component.html',
    styleUrls: ['./rejct-transaction.component.css'],
  animations: [
...
export class RejctTransactionComponent implements OnInit {
  ...
  fetchTransaction = () =>{
    ...
    this.dbIntr
      ...
      // }
       ...
       const rejectTrxn = res.data.filter(el => el.transaction_subtype.toLowerCase().includes('rejection')) 
       if(rejectTrxn.length == 0){
          this.utility.showSnackbar('No reject transactions are available on this periods',2,true);
          return;
        }
      this.state = 'collapsed';
      this.trxnRpt = rejectTrxn;
      });
  }
  ...
  handleExport = (dt) =>{
    ...
    let worksheet = workbook.addWorksheet('REJECTTRANSACTIONDETAILSREPORT'
      ,
      {
        views:[
    ...
    worksheet.mergeCells((currentRowIdx + 1), 1, (currentRowIdx + 1), endColumnIdx,'REJECTTRANSACTIONDETAILSREPORT');
    ...
    workbook.xlsx.writeBuffer().then((data)=>{
      ...
      saveAs(blob, `REJECTTRANSACTIONDETAILSREPORT.xlsx`);
    })
  }
